...
app.get('/', async (c) => {
    return c.text(
        `(⌐⊙‿⊙) Heyyo - I'm the Post Service behind ${domain}. I handle all messages.`
    );
});
...
app.get('/mail/delete', async (c) => {
    ...
    if (key) await c.env.POST_DB.delete(key);
...
app.post('/mail/forward', async (c) => {
    ...
    // check if forwarding to fakeact.fun
    if (forward.includes(domain)) {
        return c.json({
            status: 'error',
            code: 403,
            msg: `Forwarding to ${domain} is not allowed`,
        });
    }

    // convert string back to JSON
    // @ts-ignore
    const mailCont = JSON.parse(mail);
    ...
    const res = await fetch('https://api.mailchannels.net/tx/v1/send', {
        ...
        body: JSON.stringify({
            personalizations: [
                {
                    ...
                    reply_to: { email: mailCont['from'] }, // who to reply to
                    dkim_domain: domain,
            ...
            from: {
                email: `forward@${domain}`,
                name: `Just A Temp`,
            },
            subject: '📮 Forward – ' + mailCont['subject'],
            reply_to: { email: mailCont['from'] },
            content: [
                {
                    type: 'text/plain',
                    value: mailCont['content-plain'] +
                        `\n \nForwarded by ${domain}, yet another temporary email generator.` +
                        `\nIf you believe this is a mistake or the service is being abused, please contact us at security@${domain}.`,
                },
            ],
        }),
    });
