...
export const GET: APIRoute = async ({ request, locals }: APIContext) => {
    ...
    const stmt2 = locals.runtime.env.MAIL_DB?.prepare('SELECT e.message_id, e.subject, e.sender, e.body_text, e.body_html, e.received_at, es.is_read FROM emails e JOIN email_status es ON es.email_id = e.id WHERE user_email_id = ? ORDER BY received_at DESC').bind(id);
